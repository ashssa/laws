    ...
    while (i < tree.children.length) {
      ...
      if (node.type === "heading" && node.depth === 3) {
        ...
        while (
          ...
          !(tree.children[j].type === "heading" && tree.children[j].depth <= 3)
        ) {
          j++;
        }
        ...
        const gridDiv = {
          ...
          data: {
            ...
            hProperties: {
              class:
                "grid grid-cols-1 lg:grid-cols-3 divide-y lg:divide-y-0 lg:divide-x divide-base-300 border border-base-300 rounded-xl my-8 overflow-hidden shadow-sm bg-base-100 print:table print:border-collapse print:rounded-none",
            },
          },
          children: [],
        };
        ...
        const createCol = (type, title) => {
          let styleClass =
            "p-5 prose prose-sm max-w-none print:table-cell print:border print:border-black ";
          let borderColor = "";
          let bgColor = "";
          let titleColor = "";
        ...
        siblings.forEach((sib) => {
          ...
          if (currentCol && sib.type === "paragraph") {
            ...
            if (textNodeVal === "（本條新增）" || textNodeVal === "（刪除）") {
              sib.data = {
                hProperties: {
                  class: "italic opacity-50 bg-base-200/30 p-2 rounded",
                },
              };
            }
            //  TODO: 更寬鬆的匹配方式？
            ...
            if (currentCol.data.hProperties.class.includes("bg-info/5")) {
              ...
              if (/^[一二三四五六七八九十]+、/.test(textNodeVal)) {
                ...
                /^[(（][一二三四五六七八九十]+[)）]/.test(textNodeVal)
              ) {
                indentClass =
                  " ml-[2em] pl-[2em] -indent-[2em] text-base-content/80";
              } else if (/^\d+[\.、]/.test(textNodeVal)) {
                indentClass =
                  " ml-[4em] pl-[1.5em] -indent-[1.5em] text-base-content/70";
              }

              if (indentClass) {
                sib.data = sib.data || {};
                sib.data.hProperties = sib.data.hProperties || {};
                sib.data.hProperties.class =
                  (sib.data.hProperties.class || "") + indentClass;
              }
            }
