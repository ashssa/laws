...
tags:
  ...
  # - 行政公告
  # - 設計理念
description: 「如果不拒絕那些不想做的事，就沒有時間與心力去做真正想做的事了。」本文統整法規系統未來的發展願景及展望。
---
...
就現行的法律系統，本文筆者認為仍不臻完美，還有一系列的改裝目標想要完成，這些目標現在都乖乖躺在筆者的Google Keep內。
<!-- 總有一天，一定會把他們全部實裝的！各位等我！ -->
...
1. 網站設計理念介紹（就是這個BLOG！）
   ...
   - [x] ~~說明條號標示方法（項用羅馬字、款用數字、目用括號）~~
   - [x] ~~條號旁邊的錨點（`#`）按鈕~~
   - [ ] 半透明的玻璃設計（`header`、回上方按鈕）以及失敗的地方（導覽列選單）
   - ……還有更多！

   以上內容會陸續放上本Blog，上線後可至Blog「設計理念」標籤裡看到！
...
1. 列印時的版面調整
   ...
   如導覽列隱藏＋上邊距與字型調整、~~回上方按鈕隱藏~~（已完成）、調整段落遺落字元等等。

2. [x] ~~寫一本網站維護手冊~~ <!-- ☑已完成 -->
   <!-- ~~原本是想說寫[法規系統](https://ashssa.github.io/laws/)的手冊就好了，但現在看來要多寫一份BLOG版本了。~~ -->

   > 2026.03.15 按：維護手冊不僅有了，還簡化很多！（因為升級到Astro了）

3. [x] ~~設計新的PWA小工具icon，然後修正路徑~~ 因暫時註解掉PWA小工具功能，因此暫無此需求。

## 設計型

1. 捲動動畫

   - 或許可用在法規標題或路徑，捲動的有感覺有點太多（考量議會官網[內嵌iframe頁面](https://sites.google.com/a/stu.nknush.kh.edu.tw/ashs_sp/laws/system)）。
   - 技術：參考[這個](https://developer.chrome.com/blog/new-in-web-ui-io-2024?hl=zh-tw#scroll-driven_animations)。

2. ~~[「為什麼你們就是不能加個空格呢？」](https://github.com/vinta/pangu.js/blob/master/README.md)的js版本~~ 捨棄（因為`text-autospace`性質改為正式版性質了）
...
4. ~~烤土司通知的動畫有點陽春~~ 捨棄（因現階段暫無考吐司需求）

   > 2026.03.15按：遷移到 Asrto 的過程中，烤吐司通知被犧牲掉了。但目前看來狀況還好，再觀察看看。

5. 手機版字體可以再細一點……？

   靈感來源是參考數發部的官網。且暗色主題下會更好閱讀。<!-- 而且本站就有實踐！可以拿起手機跟電腦比對看看字體粗細。 -->
